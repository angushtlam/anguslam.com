    ...
    <div>
      ...
      <main>
        ...
        <ul className="list-disc mb-4">
          <li className="ml-8">
            <Link href="https://www.youtube.com/watch?v=BB8o8-EdZY0" passHref><a className="text-blue-500 underline" rel="noopener noreferrer">Matt D'Avella, "I counted everything I own as a minimalist"</a></Link>
          </li>
          <li className="ml-8">
            <Link href="https://www.youtube.com/watch?v=3zO3xUg157c" passHref><a className="text-blue-500 underline" rel="noopener noreferrer">Exploring Alternative, "EXTREME Minimalist Living With ONLY 47 Possessions!" Interview with Rob Greenfield.</a></Link>
          </li>
          <li className="ml-8">
            <Link href="https://alecleach.com/products/the-world-is-on-fire-but-we-re-still-buying-shoes" passHref><a className="text-blue-500 underline" rel="noopener noreferrer" target="_blank">Alec Leach, "The World Is On Fire But We're Still Buying Shoes"</a></Link>
          </li>
          <li className="ml-8">
            <Link href="https://konmari.com/about-the-konmari-method/" passHref><a className="text-blue-500 underline" rel="noopener noreferrer" target="_blank">Marie Kondo, "What is the KonMari Method?"</a></Link>
          </li>
        </ul>
        <p className="mb-4">
        ...
        <p className="mb-4">
          Items logged: {items.length}<br />
          Cost today for all items: ${costTodayTotal}<br />
          Last updated: {tableLastUpdated}
        </p>
        <table className="border border-black table-auto">
          <thead>
            <tr>
              <th className="border border-black whitespace-nowrap">Item name</th>
              <th className="border border-black whitespace-nowrap">Categories</th>
              <th className="border border-black whitespace-nowrap">Cost per day since ownership</th>
              <th className="border border-black whitespace-nowrap">Days until $1/day</th>
            </tr>
          </thead>
          <tbody>
            {items.map((item: any) => {
              return (
                <tr key={item.id}>
                  <td className="border border-black whitespace-nowrap">{item.name}</td>
                  <td className="border border-black whitespace-nowrap">{item.categories.map((category: any) => <span className={`bg-${category.color}-400`} key={category.name}>{category.name}</span>)}</td>
                  <td className="border border-black">${item.costPerDay}</td>
                  <td className="border border-black">{item.daysUntilTarget > 0 ? item.daysUntilTarget : "🎉"}</td>
                </tr>
              )
            })}
          </tbody>
